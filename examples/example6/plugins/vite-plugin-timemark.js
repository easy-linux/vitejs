    ...
    return {
        ...
        writeBundle: async (options, bundle) => {
            ...
            bundles.forEach((bundle) => {
                ...
                if (
                    fileregex.test(bundleFileName) &&
                    !vendorRegex.test(bundleFileName)
                ) {
                    try {
                        ...
                        const data = `/* Last build: ${zeroPad(
                            currentDate.getDate(),
                        )}:${zeroPad(
                            currentDate.getMonth(),
                        )}:${currentDate.getFullYear()}  ${zeroPad(
                            currentDate.getHours(),
                        )}.${zeroPad(currentDate.getMinutes())}.${zeroPad(
